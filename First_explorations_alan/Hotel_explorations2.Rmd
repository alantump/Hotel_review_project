...
mean(time_effct2$Estimate[time_effct2$Neg==1])
...
interaction_summary <- coefficients_table[grep(":", rownames(coefficients_table)), ]
 ...
 time_effct %>% ggplot(aes(x=Name, y= Estimate, colour = as.factor(tendency))) + 
  ...
  annotate("text", x = Inf, y = Inf, label = paste("Negative trend:", round(mean(time_effct$Neg), 2)), hjust = 1, vjust = 5+3, size = 4, color = "#F8766D") + ylim(-1,1)
```

```{r}
trans_slope = function(t_threshold){
...
threshold_slope = seq(0.5,0.975,0.025)
threshold_dif = seq(0.5,0.975,0.025)
grid = expand.grid(threshold_dif,threshold_slope, hit = NA, FA =  NA)

for (i in 1:length(grid[,1])){

  time_effct2 = trans_dif(grid[i,1])
  time_effct = trans_slope(grid[i,2])
...
# Create the heatmap
ggplot(grid, aes(x = Var1, y = Var2, fill = hit-FA, z = hit-FA)) +
  geom_tile() +
  geom_contour(color = "black") +  # Add contours
  scale_fill_gradient(low = "blue", high = "red") + # Adjust colors as needed
  labs(title = "Heatmap of Hits",
       x = "Threshold Difference",
       y = "Threshold Slope",
       fill = "Hit") +
  theme_minimal()
```
...
#mean(both$Neg ==both$Neg_label)

mean(both$Neg[both$Neg_label==1])

mean(both$Pos ==both$Pos_label)
mean(both$tendency ==both$tendency_label)
cor(both$Estimate.x,both$Estimate.y)
```


```{r}
library(caret)
cm <- confusionMatrix(factor(both$Neg, levels = c("0","1")), as.factor(both$Neg_label))
f1_neg <- cm$byClass["F1"]


cm <- confusionMatrix(factor(rbinom(length(both$Neg), 1, mean(both$Neg_label)), levels = c("0","1")), as.factor(both$Neg_label))
f1_h0 <- cm$byClass["F1"]
cat("F1 prediction:", f1_neg, "\n")
cat("F1 H0:", f1_h0, "\n")
