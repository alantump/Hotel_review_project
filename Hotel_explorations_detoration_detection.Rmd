...
# Define the observation equation (same as before)
obs <- dlmModPoly(order = 1, van = TRUE, names = c("intercept", "date_object"))

# Define the state equation (hierarchical random walk)
state <- dlmModHier(dlmModRW(), n = length(unique(train_data$Hotel_Name)))

# Combine the observation and state equations
model <- dlmMod(obs, state)

# Fit the model
fit <- dlmFilter(train_data$Reviewer_Score, model, covariates = train_data$date_object, partition = train_data$Hotel_Name)

# Obtain smoothed estimates
smooth <- dlmSmooth(fit)
```
