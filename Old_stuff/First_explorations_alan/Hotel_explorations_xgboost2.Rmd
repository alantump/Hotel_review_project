...
my_data <- my_data %>%
  mutate(Month = round(time_ref/3))  # Convert days to months

monthly_count <- my_data %>% filter( time_ref <0)  %>% mutate(R_score = round(log(Reviewer_Score*2))) %>%
  group_by(Hotel_Name, Month,R_score) %>%
  summarize(counts = length(R_score)) %>%
  ungroup()
...
final_data <- monthly_count %>%
  pivot_wider(names_from = c(Month, R_score), values_from = counts) %>% mutate(across(everything(), ~ replace_na(., 0))) %>%
...
features <- as.matrix(hotel_data %>% select(!Hotel_Name &  !difference))  # Columns -12 to 0
